...
class Manual extends Composer
{
    ...
    protected static $views = [
        'partials.content-single-manual',
    ];
    ...
    public function get_toc($content = '')
    {
        ...
        if ($items) {
            ...
            foreach ($items as $item) {
                $toc .= '<li class="mb-4 leading-tight" ><a class="text-opacity-70 text-blue" href="#' . sanitize_title_with_dashes($item[2]) . '">' . strip_tags($item[2]) . '</a></li>';
            }
            $toc .= '</ul>';
            $toc .= '</div>';
        }
...
</svg></a></div></%1$s>', $tag, $id, strip_tags($item[2]));
    ...
    private function get_tags($tag, $content = '')
    {
        ...
        // Match opening tag, content (including nested tags), and closing tag
        preg_match_all("/(<{$tag}[^>]*>)(.*?)(<\/{$tag}>)/s", $content, $matches, PREG_SET_ORDER);
