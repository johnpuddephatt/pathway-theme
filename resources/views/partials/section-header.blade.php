<div
  class="{{ $background_colour ?? 'bg-green' }} relative mb-24 flex min-h-[24rem] flex-row items-center overflow-hidden">
  ...
  <svg class="absolute -left-16 top-1/2 w-3/5 -translate-y-1/2" xmlns="http://www.w3.org/2000/svg" width="988.81"
    ...
    <path fill="#fff"
      ...
      opacity="{{ isset($background_colour) && str_contains($background_colour, 'bg-blue') ? 0.04 : 0.08 }}" />
  </svg>
  ...
  <div class="container relative w-1/2 max-w-xl flex-none py-16">
    ...
    <h1 class="mb-8 max-w-md font-serif text-6xl">{!! $title ?? get_the_title() !!}</h1>
